...
from slave.exceptions import ServiceExit
from slave.job import Job
from slave.helpers import service_shutdown
...
class slave():
    ...
    def stop(self):
        self._stop_jobs()
        self.logger.info("Slave exited.")

    def start(self, block=False):
        self.logger.info("Starting Slave..")
        self._start_jobs(block=block)

        self.logger.info("Slave now started. Jobs will run based on the interval set")
        if block:
